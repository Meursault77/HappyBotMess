...
namespace LD40.World
{
    public class Pathfinder
    {
        ...
        {
            return AStar(tiles, from, to, (t) => t.IsWalkable(), (t) => 1f);
        }
        ...
        public HashSet<Point> CalculateReach(Point from, int range, bool ignoreBlockable)
        {
            if (range == 0)
                return new HashSet<Point>();
            else
                return CalculateGenericReach(from, range, (t) => t.IsWalkable(), (t) => 1f, ignoreBlockable);
        }

        private HashSet<Point> CalculateGenericReach(Point source, int movementRange, Func<Tile, bool> IsWalkable, Func<Tile, float> WalkCost, bool ignoreBlockable = false)
        {

            Dictionary<Point, Point> prev = new Dictionary<Point, Point>();
            Dictionary<Point, float> cost = new Dictionary<Point, float>();

            List<Point> unknownNodes = new List<Point>(tiles.GetLength(0) * tiles.GetLength(1));
            ...
            while (openList.Count > 0)
            {
                ...
                if (currentNode == source || (ignoreBlockable || IsWalkable(tiles[currentNode.X, currentNode.Y])) && cost[currentNode] <= movementRange)
                {
                    closedList.Add(currentNode);
                    foreach (Point n in IterateNeighboursFourDir(currentNode.X, currentNode.Y))
                    {
                        ...
                        float tenativeCost = cost[currentNode] + WalkCost(tiles[currentNode.X, currentNode.Y]);
                        ...
                        //tenativeCost += (target - n).ToVector2().Length(); //this is only for a star
